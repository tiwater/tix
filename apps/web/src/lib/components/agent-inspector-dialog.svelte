...
<Dialog.Root bind:open={appState.showAgentInspector}>
  <Dialog.Content class="max-w-2xl w-[680px] h-[520px] flex flex-col p-0 gap-0">
    ...
    <Tabs.Root bind:value={activeTab} class="flex-1 flex flex-col min-h-0 mt-3">
      <Tabs.List class="px-6 border-b border-border rounded-none bg-transparent h-auto pb-0 justify-start gap-0 shrink-0">
        <Tabs.Trigger
          value="mind"
          class="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 pb-2.5 pt-1.5 text-xs font-medium"
        >
          <Brain size={13} class="mr-1.5" /> Mind
        </Tabs.Trigger>
        <Tabs.Trigger
          value="skills"
          class="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 pb-2.5 pt-1.5 text-xs font-medium"
        >
          <Puzzle size={13} class="mr-1.5" /> Skills
        </Tabs.Trigger>
        <Tabs.Trigger
          value="info"
          class="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 pb-2.5 pt-1.5 text-xs font-medium"
        >
          <Info size={13} class="mr-1.5" /> Info
        </Tabs.Trigger>
      </Tabs.List>
