...
*   **LLM Routing:** By overriding `ANTHROPIC_BASE_URL` with an Anthropic-compatible provider like BigModel (`LLM_API_KEY` + `LLM_BASE_URL`), the agent executes via the Claude Agent SDK directly from the Node runtime.
