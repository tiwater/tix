...
export class DiscordChannel {
    ...
    async connect() {
        ...
        this.client.on(Events.MessageCreate, async (message) => {
            ...
            if (content.startsWith('/tix')) {
                ...
                if (!content) {
                    await message.reply('Just @mention me with your task! e.g. `@Tix fix #198`');
                    return;
                }
            ...
            if (message.guild) {
                if (message.channel.isThread()) {
                    chatName = `${message.guild.name} #${message.channel.parent?.name} > ${message.channel.name}`;
                }
                else {
            ...
            if (!group) {
                ...
                if (!TRIGGER_PATTERN.test(content)) {
                    logger.debug({ chatJid, chatName }, 'Message from unregistered channel (no bot mention)');
                    return;
                }
                // Auto-register this channel
                logger.info({ chatJid, chatName }, 'Auto-registering channel on bot mention');
                const channelName = message.channel instanceof TextChannel
            ...
            logger.info({ chatJid, chatName, sender: senderName, content }, 'Discord message stored');
        });
    ...
    async createChannel(fromJid, channelName) {
        if (!this.client)
            return null;
        try {
            ...
            if (!sourceChannel ||
                ...
                !sourceChannel.guild) {
                logger.warn({ fromJid }, 'Cannot create channel: source is not a guild channel');
                return null;
            }
            ...
            logger.error({ err: err.message, fromJid, channelName }, 'Failed to create Discord channel');
            return null;
        }
    }
    async sendMessageReturningId(jid, text) {
        if (!this.client)
            return null;
        try {
            const channelId = jid.replace(/^dc:/, '');
