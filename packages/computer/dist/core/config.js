...
import fs from 'fs';
import { readEnvFile, readModelsConfig } from './env.js';
...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || envConfig.ASSISTANT_NAME || 'Shaw';
...
export const TIX_CODING_CLI = process.env.TIX_CODING_CLI || envConfig.TIX_CODING_CLI || 'gemini';
// Comma-separated sender IDs that can perform privileged mind operations.
// Example: MIND_ADMIN_USERS="ou_xxx,dc:user:12345"
export const MIND_ADMIN_USERS = (process.env.MIND_ADMIN_USERS ||
...
export const MIND_LOCK_MODE = process.env.MIND_LOCK_MODE || envConfig.MIND_LOCK_MODE || 'admin_override';
...
export let MOUNT_ALLOWLIST_PATH = path.join(TIX_HOME, 'mount-allowlist.json');
...
export function configureTixComputer(options) {
    if (options.dataDir) {
...
export function initializeDataDirs() {
    if (!fs.existsSync(TIX_HOME))
        fs.mkdirSync(TIX_HOME, { recursive: true });
    if (!fs.existsSync(AGENTS_DIR))
        fs.mkdirSync(AGENTS_DIR, { recursive: true });
}
export const SECURITY_TRUSTED_REMOTE_HOSTS = parseStringList(process.env.SECURITY_TRUSTED_REMOTE_HOSTS ||
    envConfig.SECURITY_TRUSTED_REMOTE_HOSTS, []);
export const SECURITY_ALLOW_INSECURE_REMOTE_ENDPOINTS = parseBoolean(process.env.SECURITY_ALLOW_INSECURE_REMOTE_ENDPOINTS ||
    envConfig.SECURITY_ALLOW_INSECURE_REMOTE_ENDPOINTS, false);
export const WORKSPACE_ALLOWED_ROOTS = parsePathList(process.env.WORKSPACE_ALLOWED_ROOTS || envConfig.WORKSPACE_ALLOWED_ROOTS, [HOME_DIR]);
export const CHILD_ENV_ALLOWLIST = parseStringList(process.env.CHILD_ENV_ALLOWLIST || envConfig.CHILD_ENV_ALLOWLIST, []);
...
export const SKILLS_CONFIG = {
    directories: parsePathList(process.env.SKILLS_DIRS || envConfig.SKILLS_DIRS, defaultSkillDirectories),
    adminOnly: parseBoolean(process.env.SKILLS_ADMIN_ONLY || envConfig.SKILLS_ADMIN_ONLY, true),
    allowLevel3: parseBoolean(process.env.SKILLS_ALLOW_LEVEL3 || envConfig.SKILLS_ALLOW_LEVEL3, true),
    autoEnableOnInstall: parseBoolean(process.env.SKILLS_AUTO_ENABLE || envConfig.SKILLS_AUTO_ENABLE, false),
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'tix-agent:latest';
...
// HTTP SSE channel
export const HTTP_PORT = parseInt(process.env.PORT || process.env.HTTP_PORT || envConfig.HTTP_PORT || '2756', 10);
export const HTTP_ENABLED = (process.env.HTTP_ENABLED ?? envConfig.HTTP_ENABLED ?? 'true') !== 'false';
export const HTTP_API_KEY = process.env.HTTP_API_KEY || envConfig.HTTP_API_KEY || '';
export const TIX_AUTH_TOKEN = process.env.TIX_AUTH_TOKEN || envConfig.TIX_AUTH_TOKEN || '';
export const WORKSPACE_ROOT = process.env.WORKSPACE_ROOT || envConfig.WORKSPACE_ROOT || '';
export const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS || envConfig.ALLOWED_ORIGINS || '';
 ...
 */
export function getModelPricing(entry) {
...
export const DEFAULT_MODEL = MODELS_REGISTRY.find((m) => m.default) ?? MODELS_REGISTRY[0];
// Computer identity — derived from hostname or manual override via TIX_COMPUTER_NAME
const rawHostname = os.hostname() || 'tix-local';
export const COMPUTER_HOSTNAME = process.env.TIX_COMPUTER_NAME || envConfig.TIX_COMPUTER_NAME || rawHostname;
/** Product branding name (e.g. "Supen", "Ticos"). Defaults to "Supen". */
export const TIX_PRODUCT_NAME = process.env.TIX_PRODUCT_NAME || envConfig.TIX_PRODUCT_NAME || 'Supen';
// ACP (Agent Communication Protocol) configuration
export const ACP_ENABLED = (process.env.ACP_ENABLED ?? envConfig.ACP_ENABLED ?? 'false') === 'true';
export const ACP_RELAY_URL = process.env.ACP_RELAY_URL || envConfig.ACP_RELAY_URL || '';
// Concurrency limits
export const CONCURRENCY_LIMIT = Math.max(1, parseInt(process.env.CONCURRENCY_LIMIT || envConfig.CONCURRENCY_LIMIT || '5', 10) || 5);
export const AGENT_CONCURRENCY_LIMIT = Math.max(1, parseInt(process.env.AGENT_CONCURRENCY_LIMIT ||
    envConfig.AGENT_CONCURRENCY_LIMIT ||
    '3', 10) || 3);
export const SESSION_CONCURRENCY_LIMIT = Math.max(1, parseInt(process.env.SESSION_CONCURRENCY_LIMIT ||
    envConfig.SESSION_CONCURRENCY_LIMIT ||
    '1', 10) || 1);
// Task executor defaults
export const TASK_DEFAULT_TIMEOUT_MS = parseInt(process.env.TASK_DEFAULT_TIMEOUT_MS ||
    envConfig.TASK_DEFAULT_TIMEOUT_MS ||
    '0', 10);
export const TASK_DEFAULT_STEP_TIMEOUT_MS = parseInt(process.env.TASK_DEFAULT_STEP_TIMEOUT_MS ||
    envConfig.TASK_DEFAULT_STEP_TIMEOUT_MS ||
    '0', 10);
export const TASK_DEFAULT_RETRY_COUNT = parseInt(process.env.TASK_DEFAULT_RETRY_COUNT ||
    envConfig.TASK_DEFAULT_RETRY_COUNT ||
    '0', 10);
export const TASK_DEFAULT_RETRY_BACKOFF_MS = parseInt(process.env.TASK_DEFAULT_RETRY_BACKOFF_MS ||
    envConfig.TASK_DEFAULT_RETRY_BACKOFF_MS ||
    '5000', 10);
// --- Convention-based agent paths ---
export function agentPaths(agentId) {
        ...
        try {
            ...
            if (config.workspace) {
                ...
                if (inAllowedRoot) {
                    ...
                    console.warn(`[security] Workspace "${requested}" is outside WORKSPACE_ALLOWED_ROOTS; falling back to default for agent "${normalizedAgentId}".`);
                }
            }
        }
        catch {
 ...
 */
export function getAgentModelConfig(agentId, silent = false) {
    let selectedModelId;
    try {
    ...
    if (selectedIdx === -1) {
        if (!silent) {
            logger.warn({ agentId, requestedModel: selectedModelId }, 'Agent requested model ID not found in config.yaml providers block. Falling back to default.');
        }
        return MODELS_REGISTRY;
    }
