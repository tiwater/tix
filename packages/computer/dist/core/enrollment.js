...
const ENROLLMENT_STATE_PATH = path.join(ENROLLMENT_DIR, 'enrollment-state.json');
...
function normalizeComputerId(input) {
    const raw = (input || '').trim();
    if (raw)
        return raw;
    return `${os.hostname()}-${crypto.randomUUID().slice(0, 8)}`;
}
function machineFingerprintMaterial() {
    const host = os.hostname();
    const platform = os.platform();
    const arch = os.arch();
    return `${host}|${platform}|${arch}`;
}
function deriveComputerFingerprint() {
...
function defaultState(computerId) {
    return {
...
export function readEnrollmentState(computerId) {
    ensureDir();
    ...
    try {
        const parsed = JSON.parse(fs.readFileSync(ENROLLMENT_STATE_PATH, 'utf-8'));
        const merged = {
...
export function writeEnrollmentState(state) {
    ensureDir();
    const next = {
        ...state,
        updated_at: nowIso(),
...
export function createEnrollmentToken(opts) {
    ...
    const ttl = Math.max(10, Math.min(30, opts?.ttlMinutes ?? DEFAULT_TOKEN_TTL_MINUTES));
...
function applyFailedAttempt(state) {
...
export function setTrustState(target, opts) {
    const state = readEnrollmentState(opts?.computerId);
    const next = {
        ...state,
        trust_state: target,
