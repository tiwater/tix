...
const TIX_CONFIG_PATH = path.join(process.env.HOME || os.homedir(), '.tix', 'config.yaml');
/**
 ...
 */
export function readConfigYaml(keys) {
    ...
    if (!fs.existsSync(TIX_CONFIG_PATH)) {
        ...
        logger.info({ path: TIX_CONFIG_PATH }, 'Created default config.yaml — edit it to set your API keys');
    }
    let doc;
    try {
 ...
 */
export function getEnabledChannelsFromConfig() {
    let doc;
    try {
        const content = fs.readFileSync(TIX_CONFIG_PATH, 'utf-8');
        doc = yaml.parse(content);
    ...
    for (const name of CONFIGURABLE_CHANNELS) {
        ...
        if (!block || typeof block !== 'object')
            continue;
        if (block.enabled === false || block.enabled === 'false')
            continue;
        enabled.push(name);
    }
    return enabled;
