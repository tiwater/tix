...
function extractScalar(input, keys) {
 ...
 */
function summarizeToolTarget(tool, target) {
    ...
    try {
        ...
        const picked = extractScalar(payload, preferredKeys) ||
...
export function progressKeyFromEvent(event) {
 ...
 */
export function parseProgressEvent(event) {
    const phase = typeof event.phase === 'string' ? event.phase : '';
    const action = typeof event.action === 'string' ? event.action : '';
    const target = typeof event.target === 'string' ? event.target : '';
    const elapsed = typeof event.elapsed_ms === 'number' && Number.isFinite(event.elapsed_ms)
        ? event.elapsed_ms
        : 0;
    const elapsed_s = Math.max(1, Math.round(elapsed / 1000));
    // Suppress streaming text and done events
    if (phase === 'stream_event' && action === 'speaking')
        return null;
    if (phase === 'done')
        return null;
    // Tool execution
    if (action.startsWith('executing_')) {
        ...
        if (tool.toLowerCase().includes('bash') && targetSummary) {
            const skillMatch = targetSummary.match(/skills\/([^/]+)\/scripts\/[^\s"]+\s*(.*)$/);
            if (skillMatch) {
 ...
 */
export function formatProgressText(info) {
 ...
 */
export function formatProgressFromEvent(event) {
