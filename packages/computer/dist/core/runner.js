...
import { AGENT_MIND_FILES, ASSISTANT_NAME, DEFAULT_LLM_MODEL, ANTHROPIC_API_KEY, CHILD_ENV_ALLOWLIST, LLM_API_KEY, LLM_BASE_URL, getAgentModelConfig, SKILLS_CONFIG, agentPaths, } from './config.js';
        ...
        try {
            ...
            if (isWarm) {
                ...
                try {
                    ...
                    logger.warn({ key, err: err.message }, 'AgentRunner: Warm streamInput failed — falling back to cold resume');
                    warm.alive = false;
                    warmSessions.delete(key);
            ...
            if (!usedWarm) {
                ...
                const spawnQuery = (modelOverride) => {
                    const spawnOpts = buildQueryOptions(systemPrompt, paths.workspace, this.state.agent_id, this.state.session_id, this.state.task_id, { ...modelOverride, permissionMode: agentPermissionMode, maxTaskTokens: agentMaxTaskTokens });
                ...
                for (let i = 0; i < agentModels.length; i++) {
                    ...
                    try {
                        ...
                        if (!isLast && isProviderError(err)) {
                            logger.warn({ err: err.message, failedModelId: m.id, nextModelId: agentModels[i + 1]?.id }, 'AgentRunner: Provider failed — retrying with next fallback model');
                            continue;
                        }
                        else {
