...
import { ASSISTANT_NAME, TIX_HOME, POLL_INTERVAL, TRIGGER_PATTERN, COMPUTER_HOSTNAME, SKILLS_CONFIG, MIND_ADMIN_USERS, configureTixComputer, initializeDataDirs, } from './core/config.js';
import './channels/index.js';
import { SkillsRegistry } from './skills/registry.js';
import { getChannelFactory, getRegisteredChannelNames, } from './channels/registry.js';
import { ensureSession, getAllChats, getAllRegisteredProjects, getAllSessions, getMessagesSince, getNewMessages, getRouterState, getSessionForAgent, initStore, cleanupStaleSessions, setRegisteredProject, setRouterState, storeChatMetadata, storeMessage, resolveFromChatJid, } from './core/store.js';
...
import { approvePairing, ensurePendingPairing, getBinding, listBindings, listPendingPairings, removeBinding, upsertBinding, } from './core/pairing.js';
...
import { parseProgressEvent, progressKeyFromEvent } from './core/progress.js';
// Global state
let registeredProjects = {};
const sessions = {}; // folder -> sessionId
const lastAgentTimestamp = {}; // chatJid -> iso
const channels = [];
const activeComputers = new Map();
...
async function processMessages(chatJid) {
    ...
    let agentId = getBinding(chatJid)?.agent_id || group.agent_id || group.folder;
    let sessionId;
    ...
    try {
        ...
        const existingBinding = isWebChannel
            ? { agent_id: agentId, kind: 'user', updated_at: new Date().toISOString() }
            : getBinding(chatJid);
        if (!existingBinding) {
            if (latestText.startsWith('/pair')) {
                ...
                if (sub === 'approve') {
                    ...
                    await sendFn(chatJid, `✅ Pairing approved for ${approved.chat_jid} -> ${boundAgentId} (code ${approved.pair_code})`);
                    return true;
                }
        ...
        if (latestText.startsWith('/pair')) {
            ...
            if (sub === 'unbind') {
                ...
                await sendFn(chatJid, removed ? `🧹 Removed binding for ${targetChatJid}` : `No binding found for ${targetChatJid}`);
                return true;
            }
        ...
        try {
            ...
            try {
                const computer = new AgentComputer(agentId, { ...session, task_id: taskId }.session_id, {
