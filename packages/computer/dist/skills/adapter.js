...
function detectDeclaredLevel(declared) {
    const joined = declared.join(' ').toLowerCase();
    if (joined.includes('level 3') || joined.includes('level3'))
        return 3;
    if (joined.includes('level 2') || joined.includes('level2'))
        return 2;
    if (joined.includes('level 1') || joined.includes('level1'))
        return 1;
    return undefined;
}
function permissionProfileForLevel(level, declared, reasons) {
    if (level === 1) {
        return {
            level,
            mode: 'read-only',
...
export function resolveSkillPermission(skill) {
    ...
    if (LEVEL_2_HINTS.some((hint) => haystacks.includes(hint)) ||
...
export function adaptOpenTixSkill(skill) {
