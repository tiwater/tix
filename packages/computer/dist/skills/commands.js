...
function formatList(json = false) {
    const registry = new SkillsRegistry();
    const skills = registry.listAvailable();
    if (json) {
        return JSON.stringify(skills.map((entry) => listEntryJson(entry)), null, 2);
    }
    if (skills.length === 0) {
        return [
...
function formatInspect(name) {
    ...
    if (skill.diagnostics.length > 0) {
        lines.push(`- diagnostics: ${skill.diagnostics.map((item) => `${item.severity}:${item.code}`).join(', ')}`);
    }
...
function resolveAuthScriptPath(entry, action) {
...
function executeAuthScript(entry, action, scriptPath) {
    const extension = path.extname(scriptPath).toLowerCase();
    const computer = extension === '.sh' ? 'bash' : 'node';
    const interactive = action !== 'status' && process.stdin.isTTY && process.stdout.isTTY;
    ...
    return {
        ...
        stdout: interactive || typeof result.stdout !== 'string'
            ? ''
            : result.stdout.trim(),
        stderr: interactive || typeof result.stderr !== 'string'
            ? ''
            : result.stderr.trim(),
    };
}
function classifyStatusResult(entry, scriptPath, execution) {
    if (!scriptPath) {
        return {
...
function formatAuthStatus(skillName, json = false) {
    const registry = new SkillsRegistry();
    if (skillName) {
    ...
    return ok([
        'Skill auth status:',
        ...results.map((result) => `- ${result.skill}: ${summarizeStatus(result)}`),
    ].join('\n'));
}
function executeAuthAction(action, skillName) {
    const registry = new SkillsRegistry();
    const entry = registry.inspectSkill(skillName);
    if (!entry) {
        return fail(`Skill "${skillName}" was not found.`);
    ...
    if (!scriptPath) {
        return fail(`Skill "${skillName}" does not implement auth ${action}. Expected one of: ${AUTH_SCRIPT_CANDIDATES[action].join(', ')}`);
    }
...
function executeAuthCommand(args, context, json = false) {
    if (!context.isAdmin) {
...
function formatAudit(json = false, limit = 20) {
    const registry = new SkillsRegistry();
...
function executeMutation(action, target, context, options = {}) {
    const registry = new SkillsRegistry();
    ...
    try {
        if (action === 'install') {
            ...
            return ok(`Installed ${record.name} (${record.version}) from ${record.sourceRef.type}. Enabled=${record.enabled}.`);
        }
        if (action === 'upgrade') {
            const record = registry.upgradeSkill(target, context, mutationOptions);
            return ok(`Upgraded ${record.name} to ${record.version}. Source=${record.sourceRef.type}.`);
        }
        if (action === 'enable') {
...
export function executeSkillsCommand(input, context = {}) {
    ...
    switch (command) {
        ...
        case 'add':
            return target
                ...
                : fail('Usage: /skills install <name|path|git+repo|npm:package> [--trust] [--hash <sha256>] [--approve]');
        case 'upgrade':
            return target
        ...
        case 'auth':
            return executeAuthCommand(parsed.args.slice(1), commandContext, parsed.json);
        case 'audit':
