...
const HASH_IGNORES = new Set(['.git', '.tix-skill.json', 'node_modules']);
export const CURRENT_SKILL_API_VERSION = '1.0.0';
export const MANAGED_SKILL_MANIFEST = '.tix-skill.json';
function normalizeRelativePath(skillDir, value) {
    if (!value)
        return undefined;
...
function loadPackageEntrypoint(skillDir) {
...
function skillLayout(skillDir, skillFilePath) {
    return {
...
export function resolveSkillApiCompatibility(declared, current = CURRENT_SKILL_API_VERSION) {
    if (!declared) {
        return {
            current,
            status: 'unspecified',
    ...
    const rangeMatch = trimmed.match(/^(>=|<=|>|<|\^|~)\s*(v?\d+(?:\.\d+){0,2})$/);
    ...
    const compatible = currentVersion.major === exactVersion.major &&
...
function splitGitRefAndPath(spec) {
    let subPath;
    let remaining = spec;
    ...
    if (hashIndex === -1)
        return { source: remaining, subPath };
    return {
...
function normalizeFileLikePath(spec) {
...
function isLikelyGitSpec(spec) {
    return (spec.startsWith('git+') ||
        spec.startsWith('git@') ||
        /^https?:\/\/.+(?:\.git)?(?:#.+)?$/i.test(spec) ||
        /^ssh:\/\/.+(?:\.git)?(?:#.+)?$/i.test(spec));
}
export function resolveSkillSourceSpec(spec) {
...
function resolveThirdPartySkillSource(spec) {
    ...
    throw new Error(`Skill source "${spec}" is not a discovered skill name and does not match supported third-party source formats. Use a local path, "git+<repo>", or "npm:<package>".`);
}
...
function materializeLocalSource(source, targetDir) {
    if (!source.path) {
    ...
    throw new Error(`Local source "${source.spec}" must be a directory or .tgz/.tar.gz archive.`);
}
function materializeGitSource(source, targetDir, options) {
    if (!source.path) {
...
function materializeNpmSource(source, targetDir, options) {
    if (!source.packageSpec) {
        throw new Error(`NPM source "${source.spec}" could not be resolved.`);
    }
    const packDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tix-skill-pack-'));
    try {
        ...
        const packRaw = execFileSync('npm', ['pack', source.packageSpec, '--json'], {
            cwd: packDir,
            encoding: 'utf-8',
            env,
        ...
        if (!tarballName) {
            throw new Error(`npm pack did not return a tarball for "${source.packageSpec}".`);
        }
...
export function materializeSkillSource(spec, options) {
    const source = resolveThirdPartySkillSource(spec);
    const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tix-skill-src-'));
    const targetDir = path.join(workspaceDir, 'skill');
    const loadedDir = source.type === 'local'
...
function readManagedSkillManifest(skillDir) {
...
export function writeManagedSkillManifest(skillDir, manifest) {
    fs.writeFileSync(path.join(skillDir, MANAGED_SKILL_MANIFEST), JSON.stringify(manifest, null, 2));
}
export function detectSkillEntrypoint(skillDir, declaredEntry) {
...
export function loadOpenTixSkillFromDirectory(skillDir) {
    const skillFilePath = path.join(skillDir, 'SKILL.md');
    const diagnostics = [];
    if (!fs.existsSync(skillFilePath)) {
        throw new Error(`No SKILL.md found in ${skillDir}`);
    }
    const { manifest, diagnostics: manifestDiagnostics } = readManagedSkillManifest(skillDir);
    ...
    const apiCompatibility = resolveSkillApiCompatibility(parsed.metadata.skillApiVersion);
    ...
    if (manifest) {
        ...
        if (currentHash !== manifest.contentHash) {
            diagnostics.push({
                ...
                message: 'Managed skill contents no longer match the recorded SHA-256. Upgrade or reinstall before enabling.',
            });
        }
    }
    return {
...
export function discoverOpenTixSkillDirectories(skillRoots) {
...
export function loadOpenTixSkillsDetailed(skillRoots) {
    const skills = [];
    const failures = [];
...
export function loadOpenTixSkills(skillRoots) {
