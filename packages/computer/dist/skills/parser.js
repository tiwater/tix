...
function sectionBody(sections, ...aliases) {
    for (const alias of aliases) {
        const section = sections[slugifyHeading(alias)];
        if (section?.body)
            return section.body;
...
function extractKeyValueLines(body) {
    const values = {};
    for (const line of body.split(/\r?\n/)) {
        const match = line.match(/^(name|description|version|entry|entrypoint|requires|install|permissions?|skill_api_version|skillapiversion)\s*:\s*(.+)$/i);
        if (!match)
            continue;
...
function extractInstallCommands(lines) {
    const commands = [];
    let inFence = false;
...
function buildMetadata(raw, body, frontmatter, sections) {
    ...
    const permissionsLines = sectionLines(sections, 'permissions', 'permission', 'access');
    const descriptionSection = sectionBody(sections, 'description', 'overview');
    const entrySection = sectionBody(sections, 'entrypoint', 'entry', 'run', 'command') || '';
    const title = extractTitle(body);
    const description = normalizeStringValue(frontmatter.description) ||
        keyValues.description ||
        descriptionSection ||
        firstParagraph(body) ||
        title;
    const entry = normalizeStringValue(frontmatter.entry) ||
        normalizeStringValue(frontmatter.entrypoint) ||
        keyValues.entry ||
        keyValues.entrypoint ||
        entrySection;
    return {
        name: normalizeStringValue(frontmatter.name) ||
            normalizeStringValue(frontmatter.skill) ||
            keyValues.name ||
            title,
        description: description?.trim() || 'OpenTix-compatible skill',
        version: normalizeStringValue(frontmatter.version) ||
            keyValues.version ||
            undefined,
        skillApiVersion: normalizeStringValue(frontmatter.skill_api_version) ||
...
export function parseOpenTixSkillMarkdown(raw) {
    ...
    if (!metadata.entry && metadata.install.length > 0) {
        diagnostics.push({
            ...
            message: 'Install steps were found but no explicit entrypoint was declared in SKILL.md.',
        });
    }
    return {
        metadata: {
