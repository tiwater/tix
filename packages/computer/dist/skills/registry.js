...
import { hashSkillDirectory, loadOpenTixSkillFromDirectory, loadOpenTixSkillsDetailed, materializeSkillSource, resolveSkillSourceSpec, writeManagedSkillManifest, } from './loader.js';
...
function permissionModeForLevel(level) {
    if (level === 1)
        return 'read-only';
    if (level === 2)
...
export class SkillsRegistry {
    config;
    constructor(config = SKILLS_CONFIG) {
        this.config = config;
    }
    getConfig() {
        return this.config;
    ...
    listAvailable() {
        ...
        for (const installed of Object.values(state.installed)) {
            if (seen.has(installed.name))
                continue;
            const failure = failures.find((entry) => path.resolve(entry.directory) === path.resolve(installed.directory));
            entries.push({
    ...
    getSkill(name) {
        return this.loadDiscoveredSkills().skills.find((entry) => entry.name === name);
    }
    getInstalled(name) {
        return this.readState().installed[name];
    }
    installSkill(nameOrSource, context, options = {}) {
    ...
    removeSkill(name, context) {
        ...
        if (installed.sourceRef.managed) {
            ...
            if (!isPathWithin(managedRoot, installed.directory)) {
                registryError(`Refusing to remove managed skill "${name}" from path outside managed root: ${installed.directory}`);
            }
    ...
    persistDiscoveredSkill(action, skill, context, options) {
        this.assertCompatible(skill);
        ...
        if (options.expectedHash && options.expectedHash !== contentHash) {
            registryError(`Skill "${skill.name}" hash mismatch. Expected ${options.expectedHash}, got ${contentHash}.`);
        }
        const state = this.readState();
        const existing = state.installed[skill.name];
        const record = this.buildInstalledRecord(skill, context, existing, contentHash, {
            ...skill.sourceRef,
        ...
        if (options.trustSource &&
            !state.trustedSources[skill.sourceRef.canonical]) {
            state.trustedSources[skill.sourceRef.canonical] = this.buildTrustedSource(skill.sourceRef.type, skill.sourceRef.canonical, context.actor);
        }
        this.writeState(state);
        this.appendAudit(action, skill, context, contentHash);
        return record;
    }
    installManagedSkill(sourceSpec, context, options, upgradeName) {
        this.assertManagementAllowed(context, upgradeName ? 'upgrade' : 'install');
        const state = this.readState();
        const resolvedSource = resolveSkillSourceSpec(sourceSpec);
        const trustedSource = !!state.trustedSources[resolvedSource.canonical] || !!options.trustSource;
        if (!trustedSource) {
            registryError(`Skill source "${resolvedSource.canonical}" is not trusted. Re-run with "--trust" to whitelist it.`);
        }
        ...
        try {
            ...
            if (upgradeName && skill.name !== upgradeName) {
                registryError(`Upgrading "${upgradeName}" resolved to "${skill.name}". Skill names must remain stable across upgrades.`);
            }
            const conflictingSkill = this.loadDiscoveredSkills().skills.find((entry) => entry.name === skill.name &&
                path.resolve(entry.directory) !==
                    path.resolve(path.join(managedRoot, sanitizeDirectoryName(skill.name))));
            if (conflictingSkill && !conflictingSkill.sourceRef.managed) {
                registryError(`Skill name "${skill.name}" conflicts with an existing local skill at ${conflictingSkill.directory}.`);
            }
            this.assertCompatible(skill);
            this.assertExecutableActionAllowed(skill, context, upgradeName ? 'upgrade' : 'install');
            const contentHash = hashSkillDirectory(materialized.skillDir);
            if (options.expectedHash && options.expectedHash !== contentHash) {
                registryError(`Skill "${skill.name}" hash mismatch. Expected ${options.expectedHash}, got ${contentHash}.`);
            }
            stageDir = path.join(managedRoot, `.staging-${sanitizeDirectoryName(skill.name)}-${Date.now()}`);
            ...
            const finalDir = path.join(managedRoot, sanitizeDirectoryName(skill.name));
            ...
            const finalized = adaptOpenTixSkill(loadOpenTixSkillFromDirectory(finalDir));
            const existing = state.installed[finalized.name];
            state.installed[finalized.name] = this.buildInstalledRecord(finalized, context, existing, contentHash, finalized.sourceRef);
            if (!state.trustedSources[resolvedSource.canonical]) {
                state.trustedSources[resolvedSource.canonical] =
                    this.buildTrustedSource(resolvedSource.type, resolvedSource.canonical, context.actor);
            }
            this.writeState(state);
            this.appendAudit(upgradeName ? 'upgrade' : 'install', finalized, context, contentHash);
    ...
    buildInstalledRecord(skill, context, existing, contentHash, sourceRef = skill.sourceRef) {
    ...
    buildTrustedSource(type, canonical, actor) {
        return {
            type,
            canonical,
    ...
    restoreInstalledSkill(installed, failure) {
    ...
    requireSkill(name) {
        ...
        if (!skill) {
            registryError(`Skill "${name}" was not found. Checked: ${this.discoveryRoots().join(', ')}`);
        }
        return skill;
    }
    assertCompatible(skill) {
        if (skill.apiCompatibility.status === 'incompatible') {
            registryError(`Skill "${skill.name}" is not compatible with skill API ${skill.apiCompatibility.current}. ${skill.apiCompatibility.reason}`);
        }
    }
    assertManagementAllowed(context, action) {
        if (this.config.adminOnly && !context.isAdmin) {
            registryError(`Only admin users can ${action} skills.`);
        }
    }
    assertExecutableActionAllowed(skill, context, action) {
        this.assertManagementAllowed(context, action);
        if (skill.permission.level === 3) {
            if (!this.config.allowLevel3) {
                registryError(`Level 3 skills are disabled by config. Set skills.allow_level3=true to permit "${skill.name}".`);
            }
            if (!context.isAdmin) {
                registryError(`Only admin users can ${action} Level 3 skills.`);
            }
            if (!context.approveLevel3) {
                registryError(`Level 3 skill "${skill.name}" requires explicit approval. Re-run with "--approve".`);
            }
        }
    }
    readState() {
        try {
            ...
            const installedEntries = Object.entries(parsed.installed || {}).map(([name, value]) => [name, this.normalizeInstalledRecord(name, value)]);
            return {
                version: 2,
    ...
    normalizeInstalledRecord(name, value) {
        const now = new Date().toISOString();
    ...
    appendAudit(action, skill, context, contentHash) {
        const event = {
            action,
            actor: context.actor,
    ...
    discoveryRoots() {
        return Array.from(new Set([...this.config.directories, this.managedSkillsRoot()].map((entry) => path.resolve(entry))));
    }
    loadDiscoveredSkills() {
        const { skills, failures } = loadOpenTixSkillsDetailed(this.discoveryRoots());
        return {
