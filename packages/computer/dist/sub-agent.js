...
export class DelegationDepthExceededError extends Error {
    constructor(depth) {
        super(`Sub-agent delegation depth ${depth} exceeds maximum of ${MAX_DELEGATION_DEPTH}`);
...
export async function delegateToAgent(opts) {
    ...
    // Poll for completion
    const result = await pollTaskCompletion(task.id, timeoutMs, startMs, opts.signal);
    logger.info({
        taskId: task.id,
...
async function pollTaskCompletion(taskId, timeoutMs, startMs, signal) {
    while (true) {
        if (signal?.aborted) {
            return {
                taskId,
        ...
        if (!task) {
            ...
            logger.error({ taskId }, 'sub-agent: delegated task disappeared without terminal status — treating as failed');
            return {
                taskId,
                status: 'failed',
