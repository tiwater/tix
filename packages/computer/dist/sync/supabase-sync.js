...
import { ensureSession, getAllRegisteredProjects, getAllRouterState, getAllSessions, setRegisteredProject, setRouterState, } from '../core/store.js';
import { readEnvFile } from '../core/env.js';
import { logger } from '../core/logger.js';
const STORAGE_BUCKET = 'tix';
...
export function isSupabaseConfigured() {
    ...
    const enabled = process.env.SUPABASE_SYNC_ENABLED === 'true' ||
...
export function startPeriodicSupabasePush() {
    if (!isSupabaseConfigured())
        return;
    if (_periodicTimer)
        return;
...
function readAgentMindFile(dir, filename) {
...
async function pushAgentFiles(supabase) {
    if (!fs.existsSync(AGENTS_DIR))
        return;
    const uploadFile = async (storageKey, content) => {
    ...
    for (const ent of entries) {
        ...
        for (const filename of AGENT_MIND_FILES) {
            ...
            if (content)
                await uploadFile(`${STORAGE_AGENTS_PREFIX}/${ent.name}/${filename}`, content);
        }
    }
}
...
export async function pullFromSupabase() {
    ...
    try {
        ...
        logger.warn({ err }, 'Supabase pull failed (agent continues from local copy)');
    }
}
async function pullAgentFile(supabase, storageKey, outPath) {
...
async function pullAgentFiles(supabase) {
    fs.mkdirSync(AGENTS_DIR, { recursive: true });
    const pullWithLegacy = async (filename, dir, agentsPrefix, groupsPrefix) => {
    ...
    for (const filename of AGENT_MIND_FILES) {
        ...
        const ok = (await pullAgentFile(supabase, `${STORAGE_AGENTS_PREFIX}/${filename}`, outPath)) ||
    ...
    for (const folder of folders) {
        ...
        for (const filename of AGENT_MIND_FILES) {
            await pullWithLegacy(filename, agentDir, `${STORAGE_AGENTS_PREFIX}/${folder}`, `${STORAGE_GROUPS_PREFIX}/${folder}`);
        }
    }
}
//# sourceMappingURL=supabase-sync.js.map