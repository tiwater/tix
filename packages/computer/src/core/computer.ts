...
// ══════════════════════════════════════════════════════════════
// AgentComputer — Public API (unchanged interface)
// ══════════════════════════════════════════════════════════════
...
export class AgentComputer {
  ...
  async run(
    ...
    if (this.state.status === 'busy') {
      throw new Error(
        `Computer ${this.state.agent_id}:${this.state.session_id} is already busy.`,
      );
    }
    ...
    logger.info(
      ...
      'AgentComputer: Starting task loop',
    );
    ...
    try {
      ...
      if (isWarm) {
        ...
        try {
          ...
          logger.warn(
            { key, err: err.message },
            'AgentComputer: Warm streamInput failed — falling back to cold resume',
          );
          warm!.alive = false;
          warmSessions.delete(key);
          ...
          try {
             ...
             if (!isLast && isProviderError(err)) {
               logger.warn(
                 ...
                 'AgentComputer: Provider failed — retrying with next fallback model',
               );
               continue;
             } else {
