...
import type { NewMessage, ComputerState } from './types.js';
...
/** A pending message that arrived while the computer was busy. */
...
export class Dispatcher {
  private computers = new Map<string, AgentComputer>();
  ...
  async dispatch(chatJid: string, msg: NewMessage): Promise<void> {
    ...
    if (!computer) {
      logger.info(
        { agent_id, session_id },
        'Dispatcher: Spawning new AgentComputer for session',
      );
      computer = new AgentComputer(agent_id, session_id, {
        onStateChange: (state: ComputerState) => {
    ...
    if (status === 'busy') {
      if (this.isUrgentInterrupt(content)) {
        ...
        logger.info(
          { agent_id, session_id },
          'Dispatcher: Computer busy — queuing message (will deliver when idle)',
        );
