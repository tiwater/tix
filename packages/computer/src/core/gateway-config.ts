...
import { TIX_HOME, COMPUTER_HOSTNAME } from './config.js';
...
export function readGatewayConfig(): GatewayConfig {
  ...
  if (process.env.TIX_GATEWAY_URL) config.gateway_url = process.env.TIX_GATEWAY_URL;
  if (process.env.TIX_GATEWAY_TRUST_TOKEN) config.trust_token = process.env.TIX_GATEWAY_TRUST_TOKEN;
