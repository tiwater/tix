...
import { COMPUTER_HOSTNAME, HTTP_API_KEY, HTTP_PORT, TIX_HOME } from './config.js';
...
export class Gateway {
  ...
  private authenticate(): void {
    ...
    const gatewayToken = this.buildGatewayToken(state.computer_id);
    ...
    if (this.config.trust_token && state.trust_state !== 'trusted') {
      ...
      this.ws?.send(JSON.stringify({
        ...
        computer_fingerprint: state.computer_fingerprint,
      }));
    } else {
      this.ws?.send(JSON.stringify({
        type: 'auth',
   ...
   */
  private buildGatewayToken(computerId: string): string | undefined {
    const secret = process.env.TIX_GATEWAY_SECRET;
    if (!secret) return undefined;
    const ts = Date.now().toString();
    const hmac = crypto.createHmac('sha256', secret).update(`${computerId}:${ts}`).digest('hex');
    return `${computerId}.${ts}.${hmac}`;
  }
  ...
  private startReporting(): void {
    ...
    this.reportingInterval = setInterval(() => {
      if (this._connected && this.ws?.readyState === WebSocket.OPEN) {
        ...
        try {
          const stats = fs.statfsSync(TIX_HOME);
      ...
      if (payload.type === 'enrollment_result') {
        if (payload.ok) {
          ...
          verifyEnrollmentToken({
            token: this.config.trust_token!,
            computerFingerprint: payload.computer_fingerprint,
  ...
  private async handleApiRequest(payload: {
    ...
    try {
      ...
      logger.error({ err, path: payload.path }, 'Failed to relay API request to computer');
