...
export function isUrgentInterruptMessage(content: string): boolean {
  const text = (content || '').trim();
  if (!text) return false;
  return INTERRUPT_PATTERNS.some((pattern) => pattern.test(text));
}

export function findLatestInterruptIndex<T extends InterruptLikeMessage>(
  messages: T[],
): number {
  let latestIndex = -1;
...
export function trimMessagesAfterInterrupt<T extends InterruptLikeMessage>(
  messages: T[],
): T[] {
