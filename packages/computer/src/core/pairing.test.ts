...
const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'tix-pairing-'));
process.env.HOME = tmpHome;
process.env.TIX_HOME = path.join(tmpHome, '.tix');

const mod = await import('./pairing.js');

describe('pairing store', () => {
  beforeEach(() => {
    fs.rmSync(process.env.TIX_HOME!, { recursive: true, force: true });
  });
