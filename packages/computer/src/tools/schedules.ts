...
export const scheduleTools = {
   ...
   */
  async create_schedule(ctx: ToolContext, args: { prompt: string; cron: string }) {
    if (!ctx.agent_id) throw new Error('Security Error: Agent ID is missing from tool context');
   ...
   */
  async list_my_schedules(ctx: ToolContext) {
   ...
   * Remove a schedule.
   */
  async delete_schedule(ctx: ToolContext, args: { id: string }) {
