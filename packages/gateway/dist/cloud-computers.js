...
function createComputerId(name) {
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    const rand = Math.random().toString(36).slice(2, 8);
    return `${slug}-${rand}`;
}
function buildServiceName(computerId) {
...
export async function listCloudComputers() {
    if (!isConfigured())
        return [];
    const config = getConfig();
...
export async function launchCloudComputer(input) {
    const config = getConfig();
...
export async function deleteCloudComputer(serviceId) {
    const config = getConfig();
    await renderRequest(`/services/${serviceId}`, { method: 'DELETE' }, config.apiKey);
}
//# sourceMappingURL=cloud-computers.js.map