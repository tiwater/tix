...
function buildServiceName(nodeId) {
...
export async function listCloudNodes() {
    if (!isConfigured())
        return [];
    const config = getConfig();
...
export async function launchCloudNode(input) {
    const config = getConfig();
...
export async function deleteCloudNode(serviceId) {
    const config = getConfig();
    await renderRequest(`/services/${serviceId}`, { method: 'DELETE' }, config.apiKey);
}
//# sourceMappingURL=cloud-nodes.js.map