...
function getConfig() {
    ...
    if (!ownerId)
        throw new Error('Missing RENDER_OWNER_ID');
...
function makeRandomSecret(length = 40) {
...
function createRunnerId(name) {
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    const rand = Math.random().toString(36).slice(2, 8);
    return `${slug}-${rand}`;
}
function buildServiceName(runnerId) {
    return `${RUNNER_NAME_PREFIX}${runnerId}`;
}
async function renderRequest(path, init, apiKey) {
...
function mapRenderService(service) {
    ...
    const envMap = Object.fromEntries(envVars.filter((e) => e?.key).map((e) => [e.key, e.value]));
    ...
    const tier = (Object.entries(tierPlanMap).find(([, meta]) => meta.plan === plan)?.[0] ?? null);
...
export function getCloudRunnerMeta() {
    if (!isConfigured())
        return { configured: false, imageUrl: null, gatewayUrl: null };
...
export async function listCloudRunners() {
    if (!isConfigured())
        return [];
    const config = getConfig();
    const raw = await renderRequest('/services', { method: 'GET' }, config.apiKey);
    const items = Array.isArray(raw) ? raw : Array.isArray(raw?.services) ? raw.services : [];
    const services = items.map((item) => item.service || item);
    return services
        .filter((svc) => {
        const name = svc?.name || '';
        if (name === 'ticlaw-gateway')
            return false;
        return name.includes('ticlaw') || name.startsWith(RUNNER_NAME_PREFIX);
    })
        .map(mapRenderService);
}
export async function launchCloudRunner(input) {
    const config = getConfig();
    ...
    const imageBlock = { imagePath: imageUrl };
...
export async function deleteCloudRunner(serviceId) {
    const config = getConfig();
    await renderRequest(`/services/${serviceId}`, { method: 'DELETE' }, config.apiKey);
}
//# sourceMappingURL=cloud-runners.js.map