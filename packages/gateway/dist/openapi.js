...
export function buildOpenApiSpec(opts = {}) {
    ...
    return {
        ...
        info: {
            ...
            version: '1.0.0',
            description: 'HTTP API exposed by the TiClaw Gateway. All /api/v1/* routes are transparently relayed to the connected node. Use X-Node-Id to target a specific node when multiple are connected.',
        },
        ...
        components: {
            securitySchemes: {
                bearerAuth: {
                    type: 'http',
                    scheme: 'bearer',
                    description: 'API key set via GATEWAY_API_KEY env var. Omit if gateway is in open mode.',
                },
            },
            parameters: {
                nodeId: {
        ...
        paths: {
            ...
            '/api/v1/agents/{agent_id}/sessions/{session_id}/stream': {
                ...
                get: {
                    tags: ['Sessions'],
                    summary: 'SSE stream',
                    description: 'Server-Sent Events stream for real-time agent responses. Connect with EventSource. Each event has a `data` field containing JSON with a `type` field (`delta`, `done`, `error`, etc.).',
                    responses: {
                        '200': {
