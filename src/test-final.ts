...
async function testFinalEndToEnd() {
  ...
  fs.writeFileSync(path.join(agentDir, 'SOUL.md'), '# FINAL SOUL\nYou are a productive TiClaw agent.');
  ...
  app.on('broadcast', (data: any) => {
    console.log(`[TELEMETRY] ${data.event.agent_id} is ${data.event.status}. Activity: ${data.event.activity?.action}`);
  });
  ...
  const memoryFiles = fs.readdirSync(path.join(agentDir, 'memory')).filter(f => f.endsWith('.md'));
  console.log(`Found ${memoryFiles.length} journal files in memory/ directory.`);
