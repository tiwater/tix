#!/usr/bin/env bash
# tests/04-edge-cases.sh — How does the agent handle edge cases?
set -euo pipefail
...
result=$(send_message "你好，1加1等于几？只回答数字。" "default" "e2e-edge-$$")
...
result=$(send_message "$long_message" "default" "e2e-edge-long-$$" "90")
response=$(get_response_text "$result")
...
result=$(send_message 'What is the result of this expression: 10 > 5 && 3 < 7? Answer true or false only.' "default" "e2e-edge-special-$$")
response=$(get_response_text "$result")
