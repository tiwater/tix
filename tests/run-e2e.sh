#!/usr/bin/env bash
# tests/run-e2e.sh — E2E test orchestrator for Tix
#
# Usage:
...
cleanup() {
  ...
  if [ "$NO_SERVER" = false ]; then
    echo -e "${CYAN}Cleaning up temporary TIX_HOME: ${TIX_HOME}${NC}"
    rm -rf "$TIX_HOME"
  fi
}
trap cleanup EXIT
...
if [ "$NO_SERVER" = false ]; then
  echo -e "${BOLD}Starting Tix server on port ${TIX_PORT}...${NC}"
  HTTP_PORT="$TIX_PORT" npx tsx packages/computer/src/index.ts > /tmp/tix-e2e-server.log 2>&1 &
  SERVER_PID=$!
  ...
  while [ $elapsed -lt 30 ]; do
    if curl --max-time 8 -sf "http://localhost:${TIX_PORT}/health" > /dev/null 2>&1; then
  ...
  if [ $elapsed -ge 30 ]; then
    ...
    tail -20 /tmp/tix-e2e-server.log
    exit 1
  fi

  # ── Auto-enroll: ensure computer is trusted ──
  echo -e "${BOLD}Enrolling computer...${NC}"
  ENROLL_STATUS=$(curl --max-time 8 -sf "http://localhost:${TIX_PORT}/api/enroll/status")
  ...
  if [ "$TRUST" != "trusted" ]; then
    ...
    TOKEN_RESULT=$(curl --max-time 8 -sf -X POST "http://localhost:${TIX_PORT}/api/enroll/token" -H "Content-Type: application/json" -d '{"ttl_minutes":60}')
    TOKEN=$(echo "$TOKEN_RESULT" | python3 -c "import sys,json; print(json.load(sys.stdin)['token'])" 2>/dev/null)
    VERIFY=$(curl --max-time 8 -sf -X POST "http://localhost:${TIX_PORT}/api/enroll/verify" -H "Content-Type: application/json" -d "{\"token\":\"$TOKEN\",\"computer_fingerprint\":\"$FP\"}")
    ...
    if [ "$TRUST_AFTER" = "trusted" ]; then
      echo -e "${GREEN}Computer enrolled and trusted${NC}"
    else
      echo -e "${RED}Enrollment failed: ${VERIFY}${NC}"
      exit 1
    fi
  else
    echo -e "${GREEN}Computer already trusted${NC}"
  fi
else
  echo -e "${CYAN}Using existing server on port ${TIX_PORT}${NC}"
  if ! curl --max-time 8 -sf "http://localhost:${TIX_PORT}/health" > /dev/null 2>&1; then
    echo -e "${RED}No server running on port ${TIX_PORT}${NC}"
    exit 1
  fi
fi
...
echo ""
echo -e "${BOLD}${CYAN}╔══════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║     Tix E2E Test Suite            ║${NC}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════╝${NC}"
